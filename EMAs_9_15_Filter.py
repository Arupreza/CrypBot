...
class BinanceEMAAnalyzer:
    ...
    def count_consecutive_golden_cross_candles(self, df, max_candles=3):
        """
        Count how many consecutive candles since the golden cross occurred
        Returns: (has_recent_cross, consecutive_count, cross_start_idx)
        """
    ...
    def analyze_symbols(self, symbols, timeframe='15m', limit=200, max_cross_candles=3, max_price=None):
        ...
        Parameters:
        symbols: list - List of symbols to analyze (e.g., ['BTC', 'ETH', 'BTCUSDT'])
        timeframe: str - Candlestick timeframe (default '15m')
        ...
        for i, symbol in enumerate(symbols):
            try:
                ...
                has_cross, candles_since, cross_idx = self.count_consecutive_golden_cross_candles(
                    df, max_cross_candles
                )
                
                current_row = df.iloc[-1]
                
                # Create result entry for all analyzed symbols
                result = {
                    'symbol': normalized_symbol,
                    'current_price': current_price,
                    'ema9': current_row['ema9'],
                    'ema15': current_row['ema15'],
                    'candles_since_cross': candles_since if has_cross else None,
                    'has_golden_cross': has_cross,
                    'cross_time': df.iloc[cross_idx]['timestamp'] if cross_idx is not None else None,
                    'cross_price': df.iloc[cross_idx]['close'] if cross_idx is not None else None,
                    'current_time': current_row['timestamp'],
                    'price_above_ema9': current_row['price_above_ema9'],
                    'price_above_ema15': current_row['price_above_ema15'],
                    'price_above_both_emas': current_row['price_above_both_emas'],
                    'ema9_above_ema15': current_row['ema9_above_ema15'],
                    'ema_signal': current_row['ema_signal']
                }
    ...
    def display_results(self, results, show_all=False):
        """Return analysis results without printing"""
        if not results:
            return []
        
        # Filter for golden cross results if show_all is False
        if not show_all:
            golden_cross_results = [r for r in results if r['has_golden_cross']]
            if golden_cross_results:
                # Sort by candles since cross (most recent first)
                golden_cross_results.sort(key=lambda x: x['candles_since_cross'])
                return golden_cross_results
            else:
                return []
        else:
            return results
...
def EMAs_9_15_Filter(
    ...
    Parameters:
    symbols: list - List of symbols to analyze (e.g., ['BTC', 'ETH', 'ADA', 'BTCUSDT'])
    timeframe: str - Candlestick timeframe (default '15m')
    limit: int - Number of candles to fetch (default 200)
    max_cross_candles: int - Maximum candles since golden cross (default 3)
    max_price: float - Maximum price filter (default None for no filter)
    show_all: bool - Show all analyzed symbols or only golden cross ones (default False)
    
    Returns:
    pandas DataFrame - Filtered coins with EMA cross conditions
    """
    try:
        if not symbols:
            return pd.DataFrame()
        ...
        # Display results
        final_results = analyzer.display_results(raw_results, show_all=show_all)
        
        # Convert to DataFrame
        df_results = pd.DataFrame(final_results) if final_results else pd.DataFrame()
        
        if not df_results.empty:
            # Sort by golden cross status and then by candles since cross
            if 'candles_since_cross' in df_results.columns:
                df_results = df_results.sort_values(['has_golden_cross', 'candles_since_cross'], 
