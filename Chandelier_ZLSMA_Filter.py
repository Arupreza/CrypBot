...
def check_price_above_zlsma(symbol, length=200, timeframe='15m', limit=500):
    """
    Check if current price is above ZLSMA for a single symbol.
    ...
    Returns:
    dict or None - Dictionary with ZLSMA data if price is above ZLSMA, None otherwise
    """
...
class BinanceChandelierScanner:
    ...
    def check_chandelier_buy_signal(self, symbol, timeframe='15m', limit=200, max_buy_candles=3):
        """
        Check if a single symbol has Chandelier Exit buy signal within max_buy_candles
        ...
        Returns:
        dict or None - Dictionary with chandelier data if buy signal is active, None otherwise
        """
        try:
            # Get price data
...
def sequential_filter_coins(
    coin_list: list,
    timeframe: str = '15m',
    zlsma_length: int = 200,
    zlsma_limit: int = 500,
    chandelier_limit: int = 200,
    max_buy_candles: int = 3
) -> pd.DataFrame:
    """
    MODIFIED: Sequential filtering - First ZLSMA, then Chandelier Exit
    ...
    max_buy_candles: int - Max consecutive buy candles to filter (default 3)
    
    Returns:
    pandas DataFrame - Filtered results with both conditions met
    """
    ...
    try:
        ...
        # Step 1: Filter coins where current price is above ZLSMA 200
        print(f"Step 1: Filtering coins with price above ZLSMA({zlsma_length})...")
        zlsma_passed_coins = []
        zlsma_results = []
        
        for symbol in normalized_coins:
            zlsma_result = check_price_above_zlsma(
                symbol, 
                length=zlsma_length, 
                timeframe=timeframe, 
                limit=zlsma_limit
            ...
            if zlsma_result is not None:
                zlsma_passed_coins.append(symbol)
                zlsma_results.append(zlsma_result)
            
            time.sleep(0.1)  # Rate limiting
        
        if not zlsma_passed_coins:
            print("No coins passed ZLSMA filter.")
            return pd.DataFrame()
        
        print(f"ZLSMA filter passed: {len(zlsma_passed_coins)} coins")
        
        # Step 2: Check Chandelier Exit buy signals for coins that passed ZLSMA filter
        print(f"Step 2: Checking Chandelier Exit buy signals for ZLSMA-passed coins...")
        scanner = BinanceChandelierScanner()
        scanner.exchange.load_markets()
        
        final_results = []
        
        for symbol in zlsma_passed_coins:
            chandelier_result = scanner.check_chandelier_buy_signal(
                symbol,
                timeframe=timeframe,
                limit=chandelier_limit,
                max_buy_candles=max_buy_candles
            )
            
            if chandelier_result is not None:
                # Find corresponding ZLSMA data
                zlsma_data = next((z for z in zlsma_results if z['symbol'] == symbol), None)
                
                if zlsma_data:
                    # Merge chandelier and ZLSMA data
                    combined_result = {**chandelier_result, **{
                        'zlsma_200': zlsma_data['zlsma_200'],
                        'price_above_zlsma': zlsma_data['price_above_zlsma']
                    }}
        ...
        if not final_results:
            print("No coins passed both ZLSMA and Chandelier filters.")
            return pd.DataFrame()
        
        results_df = pd.DataFrame(final_results)
        results_df = results_df.sort_values(['buy_candles_count', 'price_above_zlsma'], ascending=[True, False])
        
        print(f"Final results: {len(results_df)} coins passed both filters")
        return results_df
        
    except Exception as e:
        print(f"Error in sequential_filter_coins: {e}")
        return pd.DataFrame()

# Updated main function with new logic
def chandelier_zlsma_filter_custom(
    ...
    """
    UPDATED: Main function with sequential filtering logic
    First checks ZLSMA, then Chandelier Exit for qualifying coins only
    
    Parameters:
    coin_list: list - List of coins to scan
    timeframe: str - Timeframe for analysis (default '15m')
    ...
    Returns:
    pandas DataFrame - Results with sequential filtering applied
    """
    return sequential_filter_coins(
        coin_list=coin_list,
        timeframe=timeframe,
        zlsma_length=zlsma_length,
        zlsma_limit=zlsma_limit,
        chandelier_limit=scan_limit,
        max_buy_candles=max_buy_candles
    )